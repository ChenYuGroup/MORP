...
for j=1:12
    ...
    Sacetoin(j+1)=Sacetoin(j)+vacetoin(j)/miug(j)*(dcwg(j+1)-dcwg(j));% initial acetoin concentration for the next step (mmol/L)
    v1(j+1)=vmaxg*Sg(j+1)/(Kmg+Sg(j+1));% glucose specific consumption rate for the next step (mmol/gDCW/h)
end
...
for i=1:68
    model_tre(i) = ecModel;
    model_tre(i) = changeRxnBounds(model_tre(i),'EX0064',-v2(i),'b');%tre
    model_tre(i) = changeRxnBounds(model_tre(i),'EX0068',0.5123*v2(i),'b');%acetate
    model_tre(i) = changeRxnBounds(model_tre(i),'EX0069',-0.0067*v2(i),'b');%citrate
    model_tre(i) = changeRxnBounds(model_tre(i),'EX0070',0.0073*v2(i),'b');%pyruvate
    model_tre(i) = changeRxnBounds(model_tre(i),'EX0027',0.116*v2(i),'b');%acetoin
    model_tre(i) = changeRxnBounds(model_tre(i),'EX0002',2.22*v2(i),'b');%lactate
